...
function MatchView({
  lobby, players, submissions, userId, token, timeLeft,
  onSubmit, onTimeUp,
}: {
  lobby: Lobby; players: LobbyPlayer[]; submissions: Submission[];
  userId: string; token: string; timeLeft: number;
  ...
  const handleEditorMount: OnMount = (_ed, monaco) => {
    ...
    monaco.editor.setTheme(theme === 'dark' ? 'compete-dark' : 'compete-light');
  };
  ...
  if (view === 'match' && lobby) {
    return (
      <MatchView
        ...
        userId={userId} token={token} timeLeft={timeLeft}
        onSubmit={handleSubmitResult} onTimeUp={handleTimeUp}
      />
    );
  }
